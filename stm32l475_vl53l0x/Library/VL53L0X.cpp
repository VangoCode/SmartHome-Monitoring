...
#include <VL53L0X.h>
#define TIMEOUT_NON_BLOCKING_BYTE 50400U
...
VL53L0X::VL53L0X(I2C &i2c_instance)
    : i2c(i2c_instance), address(ADDRESS_DEFAULT << 1), io_timeout(0) // no timeout
      ,
      did_timeout(false)
{
}
...
VL53L0X::VL53L0X(I2C &i2c_instance, Timer &timer)
    : i2c(i2c_instance), t(timer), address(ADDRESS_DEFAULT << 1), io_timeout(0) // no timeout
      ,
      did_timeout(false)
{
}
...
void VL53L0X::setDefaultAddress()
{
...
#if VL53L0X_MBED_NON_BLOCKING == 0
    t.start();
#endif /* VL53L0X_RTOS_KERNEL_MS_TICK */
...
void VL53L0X::i2c_event_cb(int event)
{
    /*
...
void VL53L0X::i2c_timeout_cb()
{
    if(i2c_event==I2C_EVENT_BUSSY) i2c_event = I2C_EVENT_TIMEOUT;
}

void VL53L0X::transferInternal(int tx_len, int rx_len)
{
    i2c_event = I2C_EVENT_BUSSY; // set
    if (i2c.transfer(address, (const char *)snd_buffer, tx_len, (char *)rec_buffer, rx_len, Callback<void(int)>(this, &VL53L0X::i2c_event_cb)) == -1)
    {
        // busy
        last_status = 1;
        return;
    ...
    timeout.attach_us(callback(this, &VL53L0X::i2c_timeout_cb), TIMEOUT_NON_BLOCKING_BYTE * (tx_len + rx_len + 3));
...
{
    snd_buffer[0] = reg;
    snd_buffer[1] = (value >> 8) & 0xFF;
    snd_buffer[2] = value & 0xFF;
...
{
    snd_buffer[0] = reg;
    snd_buffer[1] = (value >> 24) & 0xFF;
    snd_buffer[2] = (value >> 16) & 0xFF;
    snd_buffer[3] = (value >> 8) & 0xFF;
    snd_buffer[4] = value & 0xFF;
    transferInternal(5, 0);
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    uint16_t value;
    snd_buffer[0] = reg;
    transferInternal(1, 2);
    if (last_status == 0)
        value = (uint16_t)(rec_buffer[0] << 8) | rec_buffer[1];
    return value;
}
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    for (int i = 1; i < count + 1; ++i)
        snd_buffer[i] = *(src + i - 1);
    transferInternal(1 + count, 0);
}
...
{
    snd_buffer[0] = reg;
    transferInternal(1, count);
    memcpy(dst, rec_buffer, count);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
    ...
    buffer[4] = value & 0xFF;
    last_status = i2c.write(address, (const char *)buffer, 5, 0);
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
    ...
    last_status = i2c.read(address, (char *)&value, 1, 0);
    return value;
}
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    for (int i = 1; i < count + 1; ++i)
        buffer[i] = *(src + i - 1);
    last_status = i2c.write(address, (const char *)buffer, count + 1, 0);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
    const char REG[] = {reg};
    i2c.write(address, REG, 1, 0);
    last_status = i2c.read(address, (char *)dst, count, 0);
}
